...
if __name__ == "__main__":
    ...
    MIN_CHUNK_SIZE = (
        5000  # don't partition segment files with line count below this value
    )
    ...
    if segment_index is None:
        sys.exit(
            f"Error: could not get segment key {segment_key} from parameters {params}"
        )
    ...
    if (
        file_size // line_size <= chunk_size
    ...
    if output_index is None:
        sys.exit(
            f"Error: could not get output key {output_key} from parameters {params}"
        )

    if output_alignment_file_base is None:
        sys.exit(
            f"Error: could not get output alignment file base from parameters {params}"
        )
    ...
    if strand_index is None:
        sys.exit(
            f"Error: could not get strand key {strand_key} from parameters {params}"
        )
    ...
    if len(skip_pairs) > 0:
        ...
        query_key_order_table = {
            item: idx for idx, item in enumerate(query_key_order)
        }  # used for sorting
        ...
        for aggregate in aggregated_skip_pairs:
            ...
            with open(fname, "w") as f:
                ...
                for pair in sorted(
                    aggregate, key=lambda p: query_key_order_table[p[1]]
                ):  # p[1] is query key
