...
if __name__ == "__main__":
    ...
    DELETE_AFTER_CHUNKING = True # deletes original segment file after splitting
    MIN_CHUNK_SIZE = 5000 # don't partition segment files with line count below this value
    MAX_CHUNK_SIZE = 50000 # only used when segment size is being estimated
    DEBUG = False # include chosen split size in file name
    ...
    line_size = None # each char is 1 byte
    ...
    if estimated_lines <= chunk_size: # no need to sort if number of lines <= chunk_size
        print(' '.join(params), flush=True)
        exit(0)
